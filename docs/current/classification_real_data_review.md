...
|---:|---|---|---|---|
| 13 | 누아 | IATA/IA 오추출 제거 | 사용자 입력 필요 | 사용자 로컬 재분석 결과 미제공 |
| 14 | 세나 | EMR/HIS/OCS 정상 유지 | 사용자 입력 필요 | 사용자 로컬 재분석 결과 미제공 |
| 16 | 슈퍼진 | UX/UI 정상 유지 | 사용자 입력 필요 | 사용자 로컬 재분석 결과 미제공 |
| 17 | 바티에이아이 | SQL/ERD/AWS/RAG/API 정상 유지 | 사용자 입력 필요 | 사용자 로컬 재분석 결과 미제공 |

### posting_id=13 / 누아 / 웹서비스 기획자

- 사용자 재분석 결과 제공 여부: no
- 핵심 확인:
  - IATA 추출 여부: 사용자 입력 필요
  - IA 추출 여부: 사용자 입력 필요
- 판단: 사용자 입력 필요
- 근거: 사용자가 로컬 재분석 결과를 아직 제공하지 않았다. Codex는 DB/API 재분석을 직접 수행하지 않는다.
- 후속 조치: 사용자가 로컬에서 재분석 후 extracted_skills 및 skill/competency review_items에 `IATA`, 직무 맥락 없는 `IA`가 남는지 공유해야 한다.
...
- 핵심 확인:
  ...
  - HIS 유지 여부: 사용자 입력 필요
  - OCS 유지 여부: 사용자 입력 필요
  - UX/UI/스토리보드/와이어프레임 유지 여부: 사용자 입력 필요
- 판단: 사용자 입력 필요
- 근거: 사용자가 로컬 재분석 결과를 아직 제공하지 않았다. 정상 약어 회귀 여부를 임의로 추정하지 않는다.
- 후속 조치: 사용자가 로컬에서 재분석 후 `EMR`, `HIS`, `OCS`, `UX/UI`, `스토리보드`, `와이어프레임` 유지 여부를 공유해야 한다.

### posting_id=16 / 슈퍼진 / 글로벌 서비스 기획/운영

- 사용자 재분석 결과 제공 여부: no
- 핵심 확인:
  - UX/UI 유지 여부: 사용자 입력 필요
- 판단: 사용자 입력 필요
- 근거: 사용자가 로컬 재분석 결과를 아직 제공하지 않았다.
- 후속 조치: 사용자가 로컬에서 재분석 후 `UX/UI` 유지 여부와 기존 정상 skill 회귀 여부를 공유해야 한다.
...
- 핵심 확인:
  ...
  - RAG 유지 여부: 사용자 입력 필요
  - API 유지 여부: 사용자 입력 필요
  - Python 유지 여부: 사용자 입력 필요
- 판단: 사용자 입력 필요
- 근거: 사용자가 로컬 재분석 결과를 아직 제공하지 않았다. 정상 기술 약어 유지 여부를 임의로 추정하지 않는다.
- 후속 조치: 사용자가 로컬에서 재분석 후 `SQL`, `ERD`, `AWS`, `RAG`, `API`, `Python` 유지 여부를 공유해야 한다.
